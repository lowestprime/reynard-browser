...
final class TabManagerImplementation: NSObject, TabManager {
    ...
    func recoverSelectedTabForForeground() -> Bool {
        ...
        if selectedTab == nil {
            createInitialTab(openingScreen: Prefs.HomepageSettings.openingScreen)
            selectFallbackTabIfNeeded()
        }
    ...
    private func selectFallbackTabIfNeeded() {
        if !tabs(for: selectedTabMode).isEmpty {
            let index = min(
                max(selectedIndex(for: selectedTabMode), 0),
                tabs(for: selectedTabMode).count - 1
            )
    ...
    private func replaceSession(
        ...
        guard let location = tabLocation(for: tabID) else { return false }
