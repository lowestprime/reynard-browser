...
final class BookmarksViewController: UIViewController, UITableViewDataSource, UITableViewDelegate, UISearchBarDelegate, UIGestureRecognizerDelegate, UIDocumentPickerDelegate {
    ...
    private var isImportingBookmarks = false
    ...
    private func exportBookmarks() {
        DispatchQueue.global(qos: .userInitiated).async { [weak self] in
            ...
            let data = BookmarkHTMLTransfer.exportHTML(bookmarks: self.store.allBookmarks())
            let exportURL = FileManager.default.temporaryDirectory
                .appendingPathComponent(BookmarkHTMLTransfer.fileName)
    ...
    func documentPicker(_ controller: UIDocumentPickerViewController, didPickDocumentsAt urls: [URL]) {
        guard isImportingBookmarks, let url = urls.first else {
